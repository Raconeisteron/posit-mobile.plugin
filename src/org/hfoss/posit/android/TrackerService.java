...
public class TrackerService extends Service implements LocationListener {
	 ...
	 *  This method originally called post(udateDisplay) to cause the updateDisplay() 
	 *  method to be run in the UI thread, rather than
	 *  in the background. For now the tracker service does not interact with any UI,
	 *  but UI is planned for the future.  This is necessary because the background thread cannot
	 ...
	 *   **** BOTH of these TODO's have been implemented *****
	 *  
...
Log.i(TAG, result);
			        ...
			        Log.i(TAG, result);
					// if the point successfully is sent to the server it is put into the database using
					// content values
	                ContentValues resultGPSPoint = new ContentValues();


	                resultGPSPoint.put(PositDbHelper.EXPEDITION_GPS_POINT_ROW_ID, mRowId);
	                resultGPSPoint.put(PositDbHelper.GPS_POINT_LATITUDE, mLatitude);
	                resultGPSPoint.put(PositDbHelper.GPS_POINT_LONGITUDE, mLongitude);
	                resultGPSPoint.put(PositDbHelper.GPS_POINT_ALTITUDE, mAltitude);
...
//		mConnectivityMgr = (ConnectivityManager) getSystemService(CONNECTIVITY_SERVICE);
		mConnectivityMgr = (ConnectivityManager) this.getSystemService(this.CONNECTIVITY_SERVICE);
...
//			Utils.showToast(this, "Aborting Tracker: " +  
	...
	private boolean startLocationUpdateService(String provider) {
		...
		try {
			mLocationManager.requestLocationUpdates(provider, UPDATES_INTERVAL, MIN_DISTANCE_TRAVELED, this);	
		} catch (Exception e) {
