...
public class SettingsActivity extends PreferenceActivity implements OnPreferenceClickListener, OnSharedPreferenceChangeListener {
	...
	protected void onCreate(Bundle icicle) {
		...
		int resID = getResources().getIdentifier(FindPluginManager.mPreferences, "xml", "org.hfoss.posit.android");
		addPreferencesFromResource(resID);
	
		pluginPrefs = FindPluginManager.getPreferenceNamesIterator();
		Log.i(TAG, "iterator " + pluginPrefs.toString());
		Toast.makeText(this, pluginPrefs.toString(), Toast.LENGTH_SHORT);
		while (pluginPrefs.hasNext()) {
			String pluginPref = (String) pluginPrefs.next();
			Log.i(TAG, "pluginPref=" + pluginPref);
			this.findPreference((CharSequence) pluginPref).setOnPreferenceClickListener((OnPreferenceClickListener) this);
		}
		
		//this.findPreference("testpref").setOnPreferenceClickListener((OnPreferenceClickListener) this);
...
//
	...
	public boolean onPreferenceClick(Preference preference) {
		...
		while (pluginPrefs.hasNext()) {
			String pluginPref = (String) pluginPrefs.next();
			if (preference.getKey().equals(pluginPref)) {
				String className = (String) FindPluginManager.preferencesTable.get(pluginPref);
				try {
...
//		if(preference.getTitle().toString().equals("About ACDI/VOCA")){
//	        String className = (String) FindPluginManager.preferencesTable.get("testpref");
//			try {
//				Class activity = Class.forName(className);
//				Log.i(TAG, "Class = " + activity);
//				Intent intent = new Intent(this, activity);
